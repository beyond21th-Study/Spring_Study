...
부가기능은 DI를 통해 프록시 생성 과정에 주입됨.

의미 있는 로직인 트랜잭션 경계설정은 코드로 남김.

기계적인 코드인 인터페이스 구현과 위임은 자동 생성됨.

변하는 것과 변하지 않는 것을 명확히 분리한 구조임.

이제 남은 문제는 ProxyFactoryBean 설정의 반복임.
...
대표적인 확장 포인트가 BeanPostProcessor임.

빈 후처리기는 스프링 빈이 생성된 이후 시점에 개입함.

생성된 빈 오브젝트를 다시 가공할 수 있음.

프로퍼티 수정 가능함.

추가 초기화 작업 수행 가능함.

필요하면 빈 오브젝트 자체를 다른 오브젝트로 교체 가능함.

이 구조를 이용하면 스프링이 생성한 빈 대신 프록시를 빈으로 등록하는 것도 가능함.

이 역할을 수행하는 것이 자동 프록시 생성 빈 후처리기임.

DefaultAdvisorAutoProxyCreator는 어드바이저 기반의 자동 프록시 생성기임.

빈 후처리기 자체를 빈으로 등록하는 것만으로 적용됨.

빈이 생성될 때마다 스프링은 해당 빈을 DefaultAdvisorAutoProxyCreator에게 전달함.

후처리기는 등록된 모든 어드바이저의 포인트컷을 기준으로 판단함.

전달받은 빈이 프록시 적용 대상인지 검사함.

프록시 적용 대상이면 내부 프록시 생성기를 통해 프록시를 생성함.

해당 프록시에 어드바이저를 연결함.

이후 원래 생성된 빈 대신 프록시 오브젝트를 컨테이너에 반환함.

컨테이너는 반환된 프록시를 최종 빈으로 등록함.
...
클래스 필터는 해당 빈의 클래스가 적용 대상인지 판단함.

메소드 매처는 그 클래스의 어떤 메소드에 적용할지 판단함.

자동 프록시 생성 과정에서는 클래스 필터 단계가 먼저 사용됨.

클래스 필터를 통과한 빈만 프록시 후보가 됨.

그래서 결과적으로 ㅇ포인트컷이 빈을 선택하는 것처럼 보이는 현상이 발생함.

실제로는 클래스 → 메소드 순으로 적용 대상을 좁혀가는 구조임
...
NameMatchMethodPointcut은 메소드 매처만 가진 포인트컷이고, 클래스 필터는 모든 클래스를 허용함.

ProxyFactoryBean에서는 타깃이 이미 정해져 있으므로 메소드 선별만으로 충분했음.

포인트컷의 전체 기능을 쓰면 먼저 클래스 필터로 프록시 적용 대상 클래스인지 판단하고, 그다음 메소드 매처로 적용할 메소드를 판단함.

클래스 단계에서 제외되면 해당 빈에는 어드바이스가 전혀 적용되지 않음.

모든 빈을 대상으로 자동 프록시 적용 여부를 판단해야 하는 DefaultAdvisorAutoProxyCreator에서는 클래스와 메소드 선별을 모두 포함한 포인트컷, 정확히는 포인트컷과 어드바이스가 결합된 어드바이저가 필요함.

## DefaultAdvisorAutoProxyCreator의 적용

### 클래스 필터를 적용한 포인트 컷 작성

기존 메소드 이름만 비교하던 포인트 컷인 NameMatchMethodPointcut을 상속함.

메소드 이름 비교 기능은 그대로 사용함.

클래스 이름을 비교하는 ClassFilter를 추가 구현함.

프로퍼티로 전달된 이름 패턴으로 클래스 적용 여부를 판단함.
...
DefaultAdvisorAutoProxyCreator는 등록된 빈 중 Advisor를 구현한 빈을 모두 탐색함.

생성되는 모든 빈에 대해 어드바이저의 포인트컷을 적용해 프록시 대상 여부를 판단함.

적용 대상이면 프록시를 생성해 원래 빈과 교체함.
...
이제 ProxyFactoryBean에서 명시적으로 DI 하지 않고 DefaultAdvisorAutoProxyCreator에 의해 자동 수집됨.

프록시 대상 선정 과정에 참여함.

자동 생성된 프록시에 동적으로 DI 되어 동작함.

### ProxyFactoryBean 제거와 서비스 빈의 원상복구

프록시 도입 때문에 변경했던 userServiceImpl 빈 아이디를 다시 userService로 복구함.

명시적인 ProxyFactoryBean을 더 이상 등록하지 않음.

남아 있던 ProxyFactoryBean 타입 빈은 제거 가능함.
...
첫째, 트랜잭션이 필요한 빈에 트랜잭션 부가기능이 실제로 적용됐는가임.

정상 커밋 상황에서는 확인이 어려우므로 예외 발생 시 롤백 여부로 검증함.

upgradeAllOrNothing() 테스트로 이미 검증 완료함.

둘째, 트랜잭션이 필요 없는 빈에까지 적용되지는 않았는가임.

자동 프록시 생성기가 포인트컷의 클래스 필터를 기준으로 정확히 선별했는지 확인 필요함.

모든 빈을 검사하지 않고 클래스 필터 동작 여부만 간단히 검증함.

포인트컷의 클래스 이름 패턴을 변경해

testUserService 가 적용 대상에서 제외되는지 확인함.
...
이를 통해 포인트컷의 클래스 필터가 정상 동작함을 확인함.

실패 테스트를 유지할 수 없으므로 포인트컷 설정은 다시 원상복구함.
...
JDK 다이내믹 프록시로 생성된 객체는 Proxy 타입의 서브클래스임.

포인트컷 설정을 원래대로 둔 상태에서 컨테이너가 반환한 서비스 빈의 타입을 검사하는 코드를 테스트에 추가함.
...
지금까지는 클래스 이름과 메소드 이름을 각각 필터와 매처로 비교해 포인트컷을 작성했음.

조건이 복잡해질수록 리플렉션 기반 구현은 번거롭고 유지보수가 어려워짐.

스프링은 이를 해결하기 위해

표현식 기반으로 클래스·메소드를 선정하는 포인트컷 표현식(pointcut expression)

방식을 제공함.

### 포인트 컷 표현식

포인트컷 표현식을 지원하는 포인트컷을 사용하려면 AspectJExpressionPointcut을 사용함.

스프링의 포인트컷은 기본적으로 클래스 필터와 메소드 매처를 각각 제공해야 함.

기존에 사용한 NameMatchClassMethodPointcut은

클래스 이름 패턴과 메소드 이름 패턴을 서로 분리해서 비교하는 방식이었음.

그래서 클래스용 패턴과 메소드용 패턴을 각각 프로퍼티로 설정해야 했음.

AspectJExpressionPointcut은 이 과정을 단순화함.

클래스 선정과 메소드 선정을 하나의 표현식으로 동시에 지정함.

복잡한 조건도 문자열 기반 표현식으로 간결하게 작성 가능함.

이 표현식은 AspectJ 프레임워크의 포인트컷 표현식을 기반으로 함.

스프링은 이를 가져와 일부 문법을 확장해 사용함.
...
앞의 4개 메소드는 TargetInterface에 정의된 메소드를 구현한 것임.

마지막 메소드는 Target 클래스에만 정의된 메소드임.

포인트컷 표현식을 이용해 이 중 원하는 메소드만 선별하는 방법을 확인함.

클래스 선정 조건까지 함께 테스트하기 위해 비교 대상이 될 다른 클래스 하나를 추가로 준비함.

<img width="586" height="188" alt="image" src="https://github.com/user-attachments/assets/a5c79bbd-63b4-4862-8e2c-ca0814524d66" />

### 포인트 컷 표현식 문법

AspectJ 포인트컷 표현식은 **포인트컷 지시자**를 사용해 작성함.

가장 대표적으로 사용하는 지시자는 execution()임.

6장에서는 execution()지시자를 중심으로 설명함.
...
처음엔 복잡해 보이지만 메소드의 **풀 시그니처를 문자열로 매칭하는 개념**으로 보면 됨.
...
- int : 리턴 값의 타입을 지정하는 패턴임. 포인트컷 표현식에서 필수 항목임. 반드시 하나의 타입을 지정해야 함. * 를 사용해 모든 리턴 타입을 허용할 수 있음. 생략은 불가능함.
- [springbook.learningtest.spring.pointcut.Target](http://springbook.learningtest.spring.pointcut.Target) : 패키지와 클래스 또는 인터페이스 이름을 포함한 타입 패턴임. 생략 가능하며, 생략 시 모든 타입을 허용함. 뒤의 메소드 이름 패턴과 .으로 연결되므로 구분이 필요함. 패키지나 타입 이름에 * 사용 가능함. ..를 사용하면 여러 패키지를 한 번에 지정할 수 있음.
- minus : 메소드 이름 패턴. 필수 항목이므로 반드시 작성해야 함. 모든 메소드를 다 선택하는 경우 *.
- (int, int) : 메소드 파라미터의 타입을 지정하는 패턴임. 파라미터 타입을 순서대로 , 로 구분해 작성함. 파라미터가 없으면 () 로 지정함. 타입과 개수에 상관없이 허용하려면 (..) 을 사용함. ... 을 사용해 뒷부분 파 라미터 조건만 생략할 수도 있음. 필수 항목이므로 반드시 지정해야 함.
- thorws java.lang.RuntimeException : 예외 이름에 대한 타입 패턴으로 생략 가능.

포인트컷 표현식은 Method 객체가 제공하는 메소드 시그니처 구조와 동일하게 비교됨. 

따라서 이해하기 어렵지 않음. 

Target.minus() 메소드만 선택하는 포인트컷 표현식을 작성함.

이를 검증하기 위해 PointcutExpressionTest 테스트 클래스를 추가함.
...
AspectJExpressionPointcut 객체를 생성하고 expression 프로퍼티에 포인트컷 표현식을 설정하면 사용 준비가 완료됨.

포인트컷 표현식은 execution() 안에 메소드 시그니처 형태로 작성함.

 execution() 은 메소드 실행 시점을 대상으로 하는 포인트컷을 의미함.

먼저 Target.minus() 메소드로 테스트함.

클래스 필터와 메소드 매처 조건이 모두 일치하므로 결과는 true임.

Target의 다른 메소드는 클래스와 파라미터는 일치하지만 메소드 이름이나 예외 패턴이 맞지 않아 결과는 false임.

Bean.class 의 메소드는 클래스 단계부터 일치하지 않으므로 포인트컷 적용 결과는 false임.

### 포인트 컷 표현식 테스트

메소드 시그니처를 그대로 사용해 포인트컷 표현식을 구성할 수 있음.

접근제한자, 클래스 타입, 예외 패턴은 옵션이므로 생략 가능함.

필수 항목만 남기면 포인트컷 표현식을 훨씬 간단하게 작성할 수 있음.
...
다만 생략된 항목은 모두 허용 조건으로 처리됨.

접근제한자, 클래스, 예외와 무관하게 조건이 적용됨.

결과적으로 정수형 두 개의 파라미터를 받고 minus 라는 이름을 가진 메소드를 모두 선정함.
...
주어진 포인트컷으로 특정 메소드가 선택되는지 확인하는 테스트 메소드를 추가함.
